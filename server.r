...
shinyServer(
    function(input, output) {
        output$count <- renderText({length(EarthQuakesOK$mag[EarthQuakesOK$year==input$year])})
        
        output$mag <- renderText({
            if (length(EarthQuakesOK$mag[EarthQuakesOK$year==input$year])>0){
                paste('mean:',round(mean(EarthQuakesOK$mag[EarthQuakesOK$year==input$year]),digits = 2),
                      '; Std:',round(sd(EarthQuakesOK$mag[EarthQuakesOK$year==input$year]),digits = 2)
                      )
            } else {'NA'}
        })
        
        output$time <- renderText({
            if (length(TD$diff[TD$year==input$year])>0){
                paste('mean:',round(mean(TD$diff[TD$year==input$year]),digits = 2),
                      '; Std:',round(sd(TD$diff[TD$year==input$year]),digits = 2)
                      )
            } else {'NA'}
        })
        
        output$newHist <- renderPlot({getTDHist(input$year)})
        output$newLocation <- renderPlot({getPlot(input$year)})
